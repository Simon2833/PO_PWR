...
class testBoard(unittest.TestCase):
    ...
    def testIsNext(self):
        ...
        for i in range(1, count):
            ...
            x = 0
            if(i == 1):
                if(models.calc.range(models.village_base.baseList[i-1].cox, models.village_base.baseList[i-1].coy, pos[0], pos[1]) >= 3):
                    return True
            while (x != i):
                for j in range(1, i+1):
                    if(models.calc.range(models.village_base.baseList[i-j].cox, models.village_base.baseList[i-j].coy, pos[0], pos[1]) >= 3):
                        x = x + 1
                if(x != i):
                    x = 0
