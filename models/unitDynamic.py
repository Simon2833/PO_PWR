...
class unitDynamic(unit):
    ...
    def ifInBoard(cls, tab, x, y, ent, sighted, j):
        ...
        if(ent.type == "monster"):
            if(checked in [4, 5, 6]):
                unitDynamic.monsterVillager(ent, x, y, tab, j)

        elif(ent.type == "villager"):
            if(checked == 1):
                unitDynamic.villagerFood(ent, x, y, tab, j)

            if(checked in [4, 5, 6]):
                unitDynamic.villagerVillager(ent, x, y, tab, sighted, j)

            elif(checked == 3):
                unitDynamic.villagerBase(ent, x, y, tab, j)

            elif(checked == 2):
                unitDynamic.villagerMonster(ent, x, y, tab, sighted, j)
    ...
    @classmethod
    def villagerVillager(cls, ent, x, y, tab, sighted, j):
    ...
    @classmethod
    def villagerFood(cls, ent, x, y, tab, j):
        if(j > ent.range): return
    ...
    @classmethod
    def villagerBase(cls, ent, x, y, tab, j):
    ...
    @classmethod
    def villagerMonster(cls, ent, x, y, tab, sighted, j):
    ...
    @classmethod
    def monsterVillager(cls, ent, x, y, tab, j):
        for villagerlist in models.villageBase.baseList:
            for villager in villagerlist.populationList:
                if(ent.cox + x != villager.cox or ent.coy + y != villager.coy): continue
                if(j > ent.range): continue
